...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        if not self.is_decoder:
            ...
            dim_common = 768
            self._linear_1 = nn.Linear(visual_feature_dim, dim_common)
            self._linear_2 = nn.Linear(visual_feature_dim, dim_common)
            self._linear_3 = nn.Linear(text_feature_dim, dim_common)
            self._multi_head_attn = nn.MultiheadAttention(dim_common, 8)
    ...
    def forward(
        ...
        #Our mod
        if not self.is_decoder:
            K = self._linear_1(aux_image_features).transpose(0, 1)          #细粒度视觉信息交互
            # V = self._linear_2(image_features).transpose(0, 1)            #粗粒度视觉信息交互
            ...
            # output = self._linear_4(torch.cat((hidden_states, attn_output), 2))         #Add
            ###mod
            output = attn_output
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config, v_encoder=None):
        super().__init__(config)
        
        self.visual_encoder = v_encoder.encode_image
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        self, input_ids: torch.LongTensor, visual, aux_visual, model_kwargs
    ) -> Dict[str, Any]:
        if "encoder_outputs" not in model_kwargs:
            ...
            ModelOutput = encoder(input_ids, return_dict=True, image_features=visual, aux_image_features=aux_visual, **encoder_kwargs)
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        if model_kwargs.get("attention_mask", None) is None:
            ...
            # model_kwargs_temp = self._prepare_attention_mask_for_generation(
        ...
        if self.config.is_encoder_decoder:
            visual, aux_visual = self.visual_encoder(visuals_global), self.visual_encoder(visuals_global)
            visual = visual * global_weights
            aux_visual = aux_visual * local_weights
            model_kwargs = self._prepare_encoder_decoder_kwargs_for_generation(input_ids, visual, aux_visual, model_kwargs)
    ...
    def forward(
        ...
        image_features, aux_image_features = self.visual_encoder(visuals_global), self.visual_encoder(visuals_local)
        image_features, aux_image_features = image_features * global_weights, aux_image_features * local_weights
        use_cache = use_cache if use_cache is not None else self.config.use_cache
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        ...
        if encoder_outputs is None:
            ...
            encoder_outputs = self.encoder(
                ...
                aux_image_features=aux_image_features,
            )
        elif return_dict and not isinstance(encoder_outputs, BaseModelOutput):
            encoder_outputs = BaseModelOutput(
                last_hidden_state=encoder_outputs[0],
